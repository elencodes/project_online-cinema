import"./style-K-ao8pVJ.js";import{l as m,s as d}from"./reviews-DneDnodl.js";m("reviewsCartoon");d("reviewsCartoon");const v="../../data.json",u=document.querySelector("#movieTitle"),g=document.querySelector("#movieYear"),p=document.querySelector("#movieAge"),y=document.querySelector("#movieGenre"),f=document.querySelector("#movieRating"),L=document.querySelector("#movieActors"),s=document.querySelector("#movieDescription"),h=document.querySelector("#movieDirector");async function q(){try{const e=(await(await fetch(v)).json()).find(c=>c.name==="Исповедь неполноценного человека");if(e){u.textContent=e.name,g.textContent=e.year,p.textContent=e.age,y.textContent=e.genre,f.textContent=e.rating,h.textContent=e.director,e.cast.slice(0,4).forEach(t=>{const o=document.createElement("div");o.className="rating-cast__box--style remove__item",o.innerHTML=`
                ...
                `,document.querySelector("#toggleDescriptionLink").addEventListener("click",l=>{l.preventDefault(),i(!t)})};i(!1)}else console.error("Фильм не найден")}catch(r){console.error("Ошибка при загрузке данных:",r)}}q();
