...
function saveReview(review) { //парсим и ищем по ключу 'reviews'  если ничего нет создаём пустой массив если есть добавляем отзыв и отправляем обратно
	const reviewsComedy = JSON.parse(localStorage.getItem('reviewsComedy')) || [];
	reviewsComedy.push(review);
	localStorage.setItem('reviewsComedy', JSON.stringify(reviewsComedy));
}
