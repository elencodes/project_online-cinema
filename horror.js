...
function loadReviews() {
	...
	const reviewsHorror = JSON.parse(localStorage.getItem('reviewsHorror')) || []; //получаем и парсим ниже подробнее будет
	reviewsList.innerHTML = reviewsHorror.map(review => `  
...
function saveReview(review) { //парсим и ищем по ключу 'reviews'  если ничего нет создаём пустой массив если есть добавляем отзыв и отправляем обратно
	const reviewsHorror = JSON.parse(localStorage.getItem('reviewsHorror')) || [];
	reviewsHorror.push(review);
	localStorage.setItem('reviewsHorror', JSON.stringify(reviewsHorror));
}
